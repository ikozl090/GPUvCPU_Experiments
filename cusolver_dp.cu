...
void cusolver(int dim, double times[2]) {
    ...
    double *A, *d_A, *b, *d_b; 
    long int size_A = sizeof(double) * rows_A * cols_A; 
    long int size_b = sizeof(double) * lda; 
    ...
    double max_matrix_val = (double) 1; // Make value size proportional to matrix dim to avoid having an unsolvable matrix
    ...
    **********************************/
    ...
    double *Workspace; // Buffer used during LU decomposition  
    int lwork = 0; // Size of workspace buffer used for operation 

    // Compute buffer size required for LU decomposition 
    cusolverDnDgetrf_bufferSize(solver_handle, rows_A, cols_A, d_A, lda, &lwork);
    ...
    cusolverDnDgetrf(solver_handle, rows_A, cols_A, d_A, lda, Workspace, devIpiv, devInfo); 

    // Solve system in LU decomposed form 
    cusolverDnDgetrs(solver_handle, CUBLAS_OP_N, n, nrhs, d_A, lda, devIpiv, d_b, lda, devInfo); 
    ...
    // Copy results to CPU 
    double* x = (double*)malloc(rows_A * sizeof(double)); 
    cudaMemcpy(x, d_b, sizeof(double) * rows_A, cudaMemcpyDeviceToHost); 
    ...
    **********************************/
    ...
    setenv("OPENBLAS_NUM_THREADS", "1", 1); // Only use one thread for CPU computation 
    info = LAPACKE_dgetrs(LAPACK_COL_MAJOR, 'N', n, nrhs, A, lda, ipiv, b, ldb);
...
int main(int argc, char *argv[]) {
    ...
    std::ofstream outFile("data_double_precision.csv", std::ios::app);
